...
// عناصر الرسم لكل فئة (بدون خصائص الخط — تُورَّث من المجموعة)
function shapes(k: CategoryKey) {
  switch (k) {
...
// أيقونة Duotone: طبقة توهّج عريضة خفيفة + خط واضح فوقها — ترث اللون من currentColor
export default function CategoryIcon({ k, size = 26 }: { k: CategoryKey; size?: number }) {
  const inner = shapes(k);
  const common = {
    fill: "none",
    stroke: "currentColor",
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden>
      <g {...common} strokeWidth={4} opacity={0.16}>{inner}</g>
      <g {...common} strokeWidth={1.7}>{inner}</g>
    </svg>
  );
}
